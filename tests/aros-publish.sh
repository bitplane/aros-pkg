...
# KEYGEN without a console refuses rather than make a guessable key, KEYGEN
# in the window makes one from typed keys, MANIFEST and PUBLISH run without a
# Stack command, and the host build reads what AROS published as "ok".
# Until this test, every AROS test published on the host.
#
...
MacRW:bin/Pkg KEYGEN FILE MacRW:redirected.key >MacRW:out/keygen-noconsole.o
C:Echo \"\$RC\" >MacRW:out/keygen-noconsole.rc
C:Echo ready >MacRW:out/ready
MacRW:bin/Pkg KEYGEN FILE MacRW:typed.key
C:Echo \"\$RC\" >MacRW:out/keygen-typed.rc
MacRW:bin/Pkg MANIFEST MacRW:src/Tool NAME tool KIND application >MacRW:out/manifest.o
C:Echo \"\$RC\" >MacRW:out/manifest.rc
MacRW:bin/Pkg PUBLISH MacRW:src/Tool CHANNEL MacRW:channel NAME tool KIND application SIGN MacRW:typed.key >MacRW:out/publish.o
...
aros_started=1
w=0
while [ ! -f "$share/out/ready" ] && [ "$w" -lt 120 ]; do sleep 1; w=$((w + 1)); done
sleep 3
# 64 keys at 4 bits each, or 128 at 2: neighbours differ, so each one counts
n=0
while [ ! -f "$share/out/keygen-typed.rc" ] && [ "$n" -lt 12 ]; do
    "$control" type "qwertyuiopasdfghjklzxcvbnm" > /dev/null 2>&1
    n=$((n + 1)); sleep 1
done
w=0
...
[ "$(code keygen-noconsole)" = 17 ] && has "$O/keygen-noconsole.o" 'no random source' && [ ! -e "$share/redirected.key" ]
ok $? "KEYGEN without a console refuses (17), says why, and writes no key"
[ "$(code keygen-typed)" = 0 ] && has "$share/typed.key" '^Seed: [0-9a-f]\{64\}$'
ok $? "KEYGEN in the window makes a key from typed keys"
...
ok $? "PUBLISH runs within the default stack, signed by the typed key"
...
pub=$(awk '/^Public:/{print substr($2, 1, 16)}' "$share/typed.key" 2> /dev/null)
[ -n "$pub" ] && has "$work/show" "$pub";                      ok $? "and its signer is the typed key"
