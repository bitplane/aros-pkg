...
- `Portal:ViewKeys` (`name:sha256;...`, made by `Portal viewkey <name>`) lets a person see unlisted channels on the site: opening `/see/<key>` sets a cookie in that browser, `/see/off` removes it. It shows and does nothing else.
- `/publish`, the address Pkg's refusals name, leads to "Publish on this portal" on the publishers' page: ask the maintainer (`Portal:Policy:Contact`), what to send, by link or with files, the signed push. Every refusal about who may push names it, and `/api/policy` gives it as `publish`.
